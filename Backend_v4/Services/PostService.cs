...
namespace Project_Version1.Services
{
    public class PostService
    {
        ...
        public async Task<Post> CreatePostAsync(PostCreateDto dto, int userId, int deptId)
        {
            ...
            if (dto.Tags != null && dto.Tags.Any())
            {
                foreach (var tagName in dto.Tags
                                 ...
                                 .Distinct())
                {
                    // Try to find existing tag (from any dept)
                    var existingTag = await _db.Tags
                        .FirstOrDefaultAsync(x => x.TagName.ToLower() == tagName);

                    if (existingTag != null && existingTag.DeptId != deptId)
                    {
                        // ❌ Throw exception if tag belongs to a different department
                        throw new ArgumentException(
                            $"Tag '{tagName}' belongs to another department and cannot be used."
                        );
                    }

                    // Create new tag if not found in this dept
                    var tag = existingTag ?? new Tag { TagName = tagName, DeptId = deptId };
        ...
        public async Task<PostDetailDto?> GetPostAsync(int id)
        {
            ...
            // If reposts exist, take the latest one (you can adjust logic if needed)
            var repost = post.Reposts.OrderByDescending(r => r.CreatedAt).FirstOrDefault();
            if (repost != null)
            {
                dto.IsRepost = true;
                dto.RepostedBy = repost.User?.FullName;
            }
        ...
        public async Task<List<PostBriefDto>> GetPostsFeedAsync(int? deptId, string? tag, int page, int pageSize)
        {
            ...
            var dtos = _mapper.Map<List<PostBriefDto>>(posts);

            // Attach repost info
            foreach (var dto in dtos)
            {
                var entity = posts.First(p => p.PostId == dto.PostId);
                var repost = entity.Reposts.OrderByDescending(r => r.CreatedAt).FirstOrDefault();
                if (repost != null)
                {
                    dto.IsRepost = true;
                    dto.RepostedBy = repost.User?.FullName;
        ...
        public async Task DeletePostAsync(Post post)
        {
            ...
            //remove commits associated with this post

            var commits = _db.Commits.Where(c => c.PostId == post.PostId);
            _db.Commits.RemoveRange(commits);
