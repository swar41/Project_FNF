...
//   const [voteCounts, setVoteCounts] = useState({ upvotes: 0, downvotes: 0 })
//   const [userVote, setUserVote] = useState(null)
//   const [replies, setReplies] = useState(comment.replies ?? null)
//   const [showReplies, setShowReplies] = useState(false)
//   const [loadingReplies, setLoadingReplies] = useState(false)
//   const navigate = useNavigate()
...
//     try {
//       await api.put(
//         `/comments/${comment.commentId}${msg ? `?commitMessage=${encodeURIComponent(msg)}` : ''}`,
//         { commentText: editText }
//       )
//       setEditing(false)
//       refresh()
...
//     try {
//       setLoadingReplies(true)
//       const res = await api.get(`/comments/${comment.commentId}/replies`)
//       setReplies(res.data ?? [])
//     } catch (err) {
//       console.error("Failed to load replies:", err)
//       setReplies([])
...
//   // ✅ Ownership and manager permission check (handles string/number mix)
//   const isOwner = String(comment.userId) === String(auth?.userId)
//   const sameDept = String(auth?.departmentId) === String(comment.deptId)
//   const canManageComment = isOwner || (auth?.role === "Manager" && sameDept)
//   // console.log('canManageComment:', canManageComment, 'isOwner:', isOwner, 'sameDept:', sameDept, 'auth:', auth, 'comment:', comment)
//   console.log('entire token data:', auth)
//   console.log('comment deptId:', comment.deptId)
...
//         {isAuthenticated() && canManageComment && (
//           <div className="flex gap-2">
//             <button className="btn btn-link" onClick={() => setEditing(true)}>✏️ Edit</button>
//             <button className="btn btn-link" onClick={handleDelete}>🗑️ Delete</button>
//           </div>
//         )}
//       </div>
...
//             {showReplies
//               ? 'Hide replies'
//               : `View ${getReplyCount()} repl${getReplyCount() === 1 ? 'y' : 'ies'}`}
//           </button>
//         )}
//       </div>
...
//             style={{ width: '100%', marginBottom: '0.5rem' }}
//           />
//           <button className="btn btn-sm" onClick={handleReply} disabled={!replyText.trim()}>
//             Post Reply
//           </button>
//           <button className="btn btn-link" onClick={() => setReplying(false)}>Cancel</button>
//         </div>
...
function CommentItem({ comment, postId, canManagePost, auth, refresh }) {
  ...
  function getReplyCount() {
   ...
   const isOwner = String(comment.userId) === String(auth?.userId)
  const canManageComment = isOwner || (auth?.role === "Manager" && String(auth?.departmentId) === String(comment.deptId))
//   const isOwner = auth?.userId === comment.userId;
// const canManageComment =
//   auth?.role === 'Manager' &&
//   auth?.departmentId === comment.user.departmentId;
  console.log('canManageComment:', canManageComment, 'isOwner:', isOwner, 'auth:', auth, 'comment:', comment)  
  console.log('entire token data:', auth)
  console.log('comment deptId:', comment.deptId)
  console.log('auth?.departmentId',auth?.departmentId)
