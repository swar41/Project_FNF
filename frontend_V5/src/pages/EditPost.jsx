...
export default function EditPost() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      alert(`Failed to update post: ${err.response?.data?.message || err.message} || One or more tags belong to different departments.`)
    } finally {
      setSubmitting(false)
    }
  }
          ...
          {tagSuggestions.length > 0 && (
            <ul style={{
              ...
              {tagSuggestions.map((s, i) => (
                <li key={i} onClick={() => addTag(s.tagName)} style={{ padding: '0.25rem', cursor: 'pointer' }}>
                  {s.tagName}
                </li>
              ))}
            </ul>
